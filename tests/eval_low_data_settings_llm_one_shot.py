"""One-shot LLM evaluation on Reuters dataset.

Evaluates gpt-5-nano in one-shot mode (1 training example in prompt) on
...
Environment variables:
    ...
    - OUTPUT_DIR: path to write outputs (default outputs/llm_one_shot)
...
def save_predictions(predictions_df: pd.DataFrame, output_dir: str, 
                     dataset_name: str, dataset_hash: str, mode: str = 'one_shot'):
...
def create_llm_model(text_column: str, label_columns: List[str], 
                     ...
                     cache_dir: str = 'cache'):
    """Create one-shot LLM model (gpt-5-nano).
    ...
    return OpenAIClassifier(
        ...
        multi_label=True,
        few_shot_mode=1,  # One-shot: 1 training example in prompt
        auto_use_cache=True,
        cache_dir=cache_dir,
        auto_save_results=True,
        output_dir=output_dir,
        experiment_name=f"{experiment_name}_llm_one_shot"
    )


def evaluate_llm_oneshot(data_dir: str, output_dir: str, cache_dir: str = 'cache', 
                          eval_mode: str = 'test'):
    """Evaluate LLM in one-shot mode on validation and test sets.
    
    Args:
        data_dir: Directory containing Reuters train/val/test CSV files
        output_dir: Directory to save results
        cache_dir: Directory to cache LLM predictions
        eval_mode: Which dataset to evaluate - 'val', 'test', or 'both' (default: 'test')
    """
    print("=" * 80)
    print("One-Shot LLM Evaluation (gpt-5-nano)")
    print("=" * 80)
    ...
    # Sample one training example for one-shot learning
    df_train_oneshot = df_train.sample(n=1, random_state=42).reset_index(drop=True)
    
    print(f"Train: {len(df_train)} samples (1 used for one-shot)")
    ...
    # Show the one-shot example
    print("\n" + "=" * 80)
    print("One-Shot Training Example:")
    print("=" * 80)
    print(f"Text: {df_train_oneshot[text_column].iloc[0][:200]}...")
    print(f"Labels: {df_train_oneshot[label_columns].iloc[0].to_dict()}")
    ...
    # Create LLM model
    os.makedirs(output_dir, exist_ok=True)
    experiment_name = f"reuters_llm_one_shot_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    
    print("\n" + "=" * 80)
    print("Creating One-Shot LLM Model")
    print("=" * 80)
    print(f"Model: gpt-5-nano")
    print(f"Mode: One-shot (few_shot_mode=1)")
    ...
    if eval_mode in ['val', 'both']:
        print("\n" + "=" * 80)
        print("VALIDATION SET - One-Shot Prediction")
        print("=" * 80)
        
        print(f"\nPredicting on {len(df_val)} validation samples...")
        llm_model.set_mode('val')
        val_result = llm_model.predict(train_df=df_train_oneshot, test_df=df_val)
        ...
        print("\nValidation Metrics (One-Shot LLM):")
        ...
        save_predictions(val_predictions_df, output_dir, 'val', val_hash, 'one_shot')
    ...
    if eval_mode in ['test', 'both']:
        print("\n" + "=" * 80)
        print("TEST SET - One-Shot Prediction")
        print("=" * 80)
        
        print(f"\nPredicting on {len(df_test)} test samples...")
        llm_model.set_mode('test')
        test_result = llm_model.predict(train_df=df_train_oneshot, test_df=df_test)
        ...
        save_predictions(test_predictions_df, output_dir, 'test', test_hash, 'one_shot')
    ...
    print("\n" + "=" * 80)
    print("FINAL RESULTS - One-Shot LLM Evaluation")
    print("=" * 80)
    ...
    print("=" * 80)
    print(f"  val_<date>_{val_hash}_one_shot.csv")
    print(f"  val_<date>_{val_hash}_one_shot.json")
    print(f"  test_<date>_{test_hash}_one_shot.csv")
    print(f"  test_<date>_{test_hash}_one_shot.json")
    print(f"\nLocation: {output_dir}/")
    print("\n")
...
if __name__ == '__main__':
    ...
    output_dir = os.getenv('OUTPUT_DIR', 'outputs/llm_one_shot')
    cache_dir = os.getenv('CACHE_DIR', 'cache')
    eval_mode = os.getenv('EVAL_MODE', 'test')  # 'val', 'test', or 'both'
    
    evaluate_llm_oneshot(data_dir=data_dir, output_dir=output_dir, 
