...
if __name__ == "__main__":
    ...
    with open("articles.csv",  "r", encoding='utf-8',) as csvfile:
        ...
        for row in reader:
            labels.append(row[0])
            texts.append(row[1])
            publishing_dates.append(row[2])

    # split dataset
    trn_texts, tst_texts, trn_labels, tst_labels, trn_publishing_dates, tst_publishing_dates = train_test_split(
        texts,
        labels,
        publishing_dates,
        test_size=SPLIT,
    ...
    for i in range(len(trn_labels)):
        train.append([trn_labels[i], trn_texts[i], trn_publishing_dates[i]])
    
    for i in range(len(tst_labels)):
        test.append([tst_labels[i], tst_texts[i], tst_publishing_dates[i]])
